...
path = os.path.join("PaviaU_HSI")# change this path for your dataset
...
print('Kappa:', kappa)
sio.savemat(os.path.join(method_path, 'result', 'result.mat'), 
            {'oa': accuracy, 'aa': aa, 'kappa': kappa, 
             'ac_list': ac_list, 'matrix': matrix})
...
for i in range(imGIS.shape[0]):
    for j in range(imGIS.shape[1]):
        if imGIS[i,j] == 0:
            if test_bg:
                pred = clf.predict(im[i,j].reshape(-1,len(im[i,j])))[0]
                iG[i,j] = pred
            else:
                iG[i,j] = 0
        else:
...
plt.savefig(os.path.join(method_path, 'result', 'decode_map.png'),
            format='png', dpi=600)
plt.close()
